...
final readonly class YellowCardStatisticsBuilder extends StatisticsQueryBuilder
{
    public function applyToQuery(QueryBuilder $master_query, Closure $next): QueryBuilder
    {
        $master_query
            ->selectRaw("COALESCE(`{$this->getCTEName()}`.`total`, 0) AS `yellow_cards`")
